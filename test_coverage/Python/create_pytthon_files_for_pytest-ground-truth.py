...
for data in ds_python['test']:
    ...
    intended_test_code = remove_metadata_block(data['test'])
    test_code = f"""
{import_statements}

def test_{python_func_name}():
{extract_check_body(intended_test_code)}
"""
    with open(f"./dataset/{python_func_name}.py", "w") as f:
        f.write(function_code)
    with open(f"./dataset/tests/test_{python_func_name}.py", "w") as f:
        f.write(test_code)
